...
public class UsuarioService {
    ...
    public Long cadastrar(DadosCadastroUsuario dados){
        if (dados.login() == null || dados.senha() == null){
            throw new IllegalArgumentException("Ops... Login e senha são obrigatórios!");
        }
        if (repository.existsByLogin(dados.login())){
            throw new IllegalArgumentException("Este email já foi cadastrado! Por favor utilize outro email");
        }
    ...
    public Page<DadosDetalhamentoUsuario> getAllUsers(Pageable pageable) {
        ...
        return usuarioPage.map(usuario -> new DadosDetalhamentoUsuario(usuario.getId(), usuario.getNome(), usuario.getLogin(), usuario.getStatus()));
    }
    ...
    public void atualizar(Long usuarioId, DadosDetalhamentoUsuario dados){
        ...
        if (usuarioOptional.isEmpty()){
            throw new IllegalStateException("Usuário de ID: " + usuarioId + " não foi possivel encontrá-lo...");
        }
    ...
    public Usuario findByLogin(String login) {
        ...
        if (usuario == null) {
            throw new IllegalArgumentException("Usuário não foi encontrado com este login: " + login);
        }
        return usuario;
    }
    ...
    private DadosDetalhamentoUsuario mapToDetalhamentoDto(Usuario usuario) {
        return new DadosDetalhamentoUsuario(usuario.getId(),usuario.getNome(),usuario.getLogin(),usuario.getStatus());
    }
}
